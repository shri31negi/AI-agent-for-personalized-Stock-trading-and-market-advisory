...
export function MarketAnalysis() {
  ...
  // Fetch history when stock or timeframe changes
  useEffect(() => {
  ...
  const handleSearchKeyPress = async (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter' && searchQuery.trim()) {
      try {
        const res = await fetch(`http://localhost:5000/api/market/quote/${searchQuery.trim().toUpperCase()}`);
        if (res.ok) {
           const stock = await res.json();
           setSelectedStock(stock);
           if (!trendingStocksState.find(s => s.symbol === stock.symbol)) {
            ...
            <Tabs defaultValue="overview">
              ...
              <TabsContent value="technicals" className="space-y-4 mt-4">
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  ...
                  <div className="p-4 bg-gray-50 dark:bg-muted/30 rounded-lg">
                    <p className="text-sm text-gray-600 dark:text-gray-400 mb-1">50-Day MA</p>
                    <p className="font-semibold text-gray-900 dark:text-foreground">${technicalIndicators.ma50.toFixed(2)}</p>
                  ...
                  <div className="p-4 bg-gray-50 dark:bg-muted/30 rounded-lg">
                    <p className="text-sm text-gray-600 dark:text-gray-400 mb-1">200-Day MA</p>
                    <p className="font-semibold text-gray-900 dark:text-foreground">${technicalIndicators.ma200.toFixed(2)}</p>
              ...
              <TabsContent value="fundamentals" className="space-y-4 mt-4">
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  ...
                  <div className="p-4 bg-gray-50 dark:bg-muted/30 rounded-lg">
                    <p className="text-sm text-gray-600 dark:text-gray-400 mb-1">Market Cap</p>
                    <p className="font-semibold text-gray-900 dark:text-foreground">${(selectedStock.marketCap / 1e9).toFixed(2)}B</p>
                  </div>
                  <div className="p-4 bg-gray-50 dark:bg-muted/30 rounded-lg">
                    <p className="text-sm text-gray-600 dark:text-gray-400 mb-1">EPS (TTM)</p>
                    <p className="font-semibold text-gray-900 dark:text-foreground">${(selectedStock.price / (selectedStock.pe || 1)).toFixed(2)}</p>
                  </div>
                </div>
                ...
                <Card className="p-4 bg-gradient-to-r from-green-50 to-blue-50 dark:from-green-950/20 dark:to-blue-950/20 border-border dark:border-green-900/30">
                  ...
                  <div className="grid grid-cols-4 gap-4 mb-4">
                    ...
                    <div className="text-center">
                      <p className="text-2xl font-bold text-blue-600 dark:text-blue-500">${analystRatings.targetPrice.toFixed(2)}</p>
